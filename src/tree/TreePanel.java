...
public class TreePanel extends JPanel{
	...
	public TreePanel(MainFrame mf_) {
		mf = mf_;
		extended = "Files";
		setup();
	...
	public TreePanel(MainFrame mf_, String extended_) {
		mf = mf_;
		extended = extended_;
	    setup();
	}
...
//		}
		if(workingDir.equals(userDir)) path += "/" + filename;
		extended = sep+extended;
		settings = sep+settings;
		test = sep+test;
		pathFi = sep+"src"+sep+"pixel"+sep+"path.pxs";
		seperate = sep+"";
		themes = sep+"themes"+sep;
		...
		if((new File(path)).mkdir()) {
			...
			try {
				...
				fileNotFound(new File(path+settings));
				e.printStackTrace();
			}
			try {
				...
				fileNotFound(new File(path+extended+test));
				e.printStackTrace();
			}
			try {
				...
				fileNotFound(new File(userDir+pathFi));
				e.printStackTrace();
			}
			try {
				...
				fileNotFound(new File(path+themes+"Developers Choice.pxt"));
				e.printStackTrace();
			}
		}
	            ...
	            if (selectedNode != null && selectedbNode != null && !pop) {
	            	if (!selectedbNode.type().contains("_EXPORTED") && (selectedbNode.type().equals("PIXEL")
						...
						try {
							...
							fileNotFound(selectedbNode.file());
							e.printStackTrace();
						}
	...
	public void error() {
		fireTreeEvent(new TreeEvent(this, "An error has occured.", "Error"));
	}
	
	public void fileNotFound() {
		fireTreeEvent(new TreeEvent(this, "File not found.", "Error"));
	}
	
	public void success() {
		fireTreeEvent(new TreeEvent(this, "Action was successful.", "Info"));
	}
	
	public void error(String error) {
		fireTreeEvent(new TreeEvent(this, error + " has occured.", "Error"));
	}
	...
	public void fileNotFound(File file) {
		fireTreeEvent(new TreeEvent(this, file.getName() + " not found.", "Error"));
	}
		...
		for(int j = 0; j < lines; j++) {
			...
			for(int i = 0; i < temp.length; i++) {
				...
				if(j==0) {
					...
					if(i==0)t.classText1=Integer.parseInt(arg);
					if(i==1)t.classText2=Integer.parseInt(arg);
					if(i==2)t.classText3=Integer.parseInt(arg);
				} else if(j==7) {
					...
					if(i==0)t.functionText1=Integer.parseInt(arg);
					if(i==1)t.functionText2=Integer.parseInt(arg);
					if(i==2)t.functionText3=Integer.parseInt(arg);
				}
			}
		}
		return t;
	...
	public static String readFile (File file) {
		...
		try {
			...
			mf.treePanel.fileNotFound(file);
			e.printStackTrace();
		}
		return string;
	}
	...
	private MouseListener getMouseListener() {
	    return new MouseAdapter() {
	        ...
	        public void mousePressed(MouseEvent arg0) {
	            if(arg0.getButton() == MouseEvent.BUTTON1 && !pop){
	...
	public void addNode2(String type) {
		...
		try {
			...
			fileNotFound(new File(dir+"/"+b.type()+ending));
			e.printStackTrace();
		}
