...
import lang.core.general.TokenParser;
...
public class KeywordStyledDocument extends DefaultStyledDocument  {
	...
	private synchronized void refreshDocument() throws BadLocationException {
		...
		StructureParser.parse(TokenParser.parse(text));
		...
		final List<HiliteWord> functionList = processFunctionWords(text);
		...
		if(_mf.pixelPanel.highlight) {
			...
			StyleConstants.setForeground(_cwStyle, _mf.classText);
			...
			StyleConstants.setForeground(_cwStyle, _mf.functionText);
			for(HiliteWord word : functionList) {
				int p0 = word._position;
				setCharacterAttributes(p0, word._word.length(), _cwStyle, true);
			}
		}
	...
	private static  List<HiliteWord> processFunctionWords(String content) {
		content += " ";
		List<HiliteWord> hiliteWords = new ArrayList<HiliteWord>();
		int lastWhitespacePosition = 0;
		String word = "";
...
//				anyway however for completeless sake it is added], default)
				...
				word.trim().equals("or") || // ||
				word.trim().equals("patch") || // (basically instead of using the function, when compiles it puts the patch code where the 
//				function would be, basically a speed for memory trade of)
...
//				reference that variable)
				...
				(word.trim().length() > 3 && word.trim().substring(0,3).equals("tmp") && isInteger(word.trim().substring(3))) || // (operation result)
	...
	private static final boolean isObjectName(String word) {
		if(StructureParser.Objects.size()<1)return false;
		for(String s : StructureParser.Objects) if(s.trim().equals(word)) return true;
		return false;
	}
	
	private static final boolean isGlobalName(String word) {
		if(StructureParser.Globals.size()<1)return false;
		for(String[] sa : StructureParser.Globals) if(sa[0].trim().equals(word)) return true;
		return false;
	}
	
	private static final boolean isLocalName(String word) {
		if(StructureParser.Locals.size()<1)return false;
		for(String[] sa : StructureParser.Locals) if(sa[0].trim().equals(word)) return true;
		return false;
	}
	
	private static final boolean isFunctionName(String word) {
		if(StructureParser.Locals.size()<1)return false;
		for(String[] sa : StructureParser.Functions) if(sa[0].trim().equals(word)) return true;
		return false;
	}
}